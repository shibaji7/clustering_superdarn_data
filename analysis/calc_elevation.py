...
def beamToAzim(maxbeam, beam, boresite, bmsep, fov_dir="front"):
    ...
    beam azimuth
    '''
    phi = ((maxbeam - 1) / 2. - beam) * bmsep
    if fov_dir is "back": phi = 180.0 - phi
...
def calc_elv(beam, phi0_attr="phi0", phi0_e_attr="phi0_e", hard=None,
    ...
    --------
    elv : (np.array)
        A list of floats of the same size as the myBeam.slist list,
    ...
    assert isinstance(phi0_attr, str) and hasattr(beam, phi0_attr), \
    ...
    if hasattr(beam, phi0_e_attr):
        phi0_e = getattr(beam, phi0_e_attr)
    else:
    ...
    if hard is None:
        hard = pydarn.read_hdw_file(radId=beam.stid)
    ...
    bm_az = np.radians(beamToAzim(np.max(hard.beams), beam.bmnum, hard.boresight, hard.beam_separation, fov_dir="front") - hard.boresight)
    cos_az = np.cos(bm_az)
    sin_az = np.sin(bm_az)
    az_sign = 1.0 if hard.interferometer_offset.y > 0.0 else -1.0

    # Find the elevation angle with the maximum phase lag
    el_max = np.arcsin(az_sign * hard.interferometer_offset.z * cos_az /
                       np.sqrt(hard.interferometer_offset.y**2 + hard.interferometer_offset.z**2))
    if el_max < 0.0:
        el_max = 0.0
    ...
    k = 2.0 * np.pi * beam.tfreq * 1.0e3 / scicon.c

    # Calculate the phase shift due to the radar cables
    del_chi = -np.pi * beam.tfreq * tdiff * 2.0e-3

    # Find the maximum possible phase shift
    chimax = k * (hard.interferometer_offset.x * sin_az + hard.interferometer_offset.y *
                  np.sqrt(cos_max**2- sin_az**2) + hard.interferometer_offset.z * sin_max)
    ...
    for i,p0 in enumerate(phi0):
        ...
        if p0 != 0.0 or phi0_e[i] != 0.0:
            ...
            if phi_temp > max(chimax, chimin):
                ...
                cos_theta = phi_temp / k - hard.interferometer_offset.x * sin_az
                yz_sum2 = hard.interferometer_offset.y**2 + hard.interferometer_offset.z**2
                sin_delta = (cos_theta * hard.interfer[2] +
                             np.sqrt((cos_theta * hard.interferometer_offset.z)**2 -
                                     yz_sum2 * (cos_theta**2 -
                                                (hard.interferometer_offset.y *
                                                 cos_az)**2))) / yz_sum2
...
def calc_elv_w_err(beam, phi0_attr="phi0", phi0_e_attr="phi0_e", hard=None,
    ...
    beam : (class `pydarn.sdio.radDataTypes.beamData`)
        Data for a single radar and beam along all range gates at a given time
    phi0_attr : (str)
        Name of the phase lag attribute in the beam object (default="phi0")
    phi0_e_attr : (str)
    ...
    assert isinstance(phi0_attr, str) and hasattr(beam, phi0_attr), \
        logging.error('the phase lag data is not in this beam')
    assert isinstance(phi0_e_attr, str) and hasattr(beam, phi0_attr), \
    ...
    theta2_base = sig2_az * (hard.interferometer_offset.x * cos_az)**2
    ...
    for i,p0 in enumerate(phi0):
        ...
        if p0 != 0.0 and not np.isnan(phi0_e[i]):
            ...
            if phi_temp > max(chimax, chimin):
                ...
                cos_theta = phi_temp / k - hard.interferometer_offset.x * sin_az
                yz_sum2 = hard.interferometer_offset.y**2 + hard.interferometer_offset.z**2                
                sin_delta = (cos_theta * hard.interferometer_offset.z +
                ...
                if sin_delta <= 1.0:
                    ...
                    if sig2_az > 0.0:
                        ...
                        az_term2 *= np.power(hard.interferometer_offset.y, 4) * sig2_az
                    ...
                    if theta_term2 > 0.0:
                        theta_term2 *= ((hard.interferometer_offset.z - hard.interferometer_offset.y**2
                    ...
                    if iy_e > 0.0:
                        iy_term2 = (yz_sum2 - 2.0 * cos_theta *
                                    hard.interferometer_offset.y * hard.interferometer_offset.z - 2.0 *
                                    hard.interferometer_offset.y * np.sqrt(yscale2) +
                                    hard.interferometer_offset.y**2 * yz_sum2 *
                    ...
                    if iz_e > 0.0:
                        iz_term2 = ((yz_sum2 * (cos_theta + hard.interferometer_offset.z *
                                                (hard.interferometer_offset.y * cos_az)**2 /
                                                np.sqrt(yscale2)) - 2.0 *
                                     hard.interferometer_offset.z * np.sqrt(yscale2))
